...
def run_shovill(untrimmmed_forward_reads,untrimmmed_backward_reads):
    """
    Shovill runs multiple tools, trimming and picks best output.
    Skip running perform_qc_trimming, run_trimmomatic
    :return:
    """
    for i in range(len(untrimmmed_forward_reads)): #Assuming that all reads are paired and matched
        forward_read = untrimmmed_forward_reads[i]
        backward_read = untrimmmed_backward_reads[i]
    output_dir = f"./ga/{isolate_id}"
    input_dir = "../../../sample_inputs/"
    shovill_command = f"shovill --gsize 5M --cpus 4 --trim --outdir {output_dir} --R1 {input_dir}{isolate_id}/{isolate_id}_1.fq.gz --R2 ../../../sample_inputs/{isolate_id}/{isolate_id}_2.fq.gz --force"
    copy_command = f"cp {output_dir}/{isolate_id}/contigs.fa {output_dir}/{isolate_id}/{isolate_id}_contigs.fa"
    move_command = f"mv {output_dir}/{isolate_id}/{isolate_id}_contigs.fa ./final_outputs/{isolate_id}_contigs.fa"
...
def main():
    ...
    parser.add_argument("-o", "--output", help="Output Directory", required=False, type=str)
